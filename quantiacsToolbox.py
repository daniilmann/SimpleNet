...
#        marketFile = os.path.join('tickerData',market+'.txt')
        marketFile = os.path.join(dataDir,market+'.txt') # мое исправление
...
def runts(tradingSystem, plotEquity=True, reloadData=False, state={}, sourceData='tickerData', dataDictCommon= None):
    ...
    Returns:
        ...
        keys and description:
            ...
            'tools' (dict): Performance numbers of the backtest
    ...
    if 'updateSettings' in state:
        if isinstance(state['updateSettings'], dict):
            settings.update(state['updateSettings'])
        else:
            print 'Can not update settings, wrong format'
    ...
    if dataDictCommon is not None:
        dataDict=deepcopy(dataDictCommon)
        dataCache=deepcopy(dataDictCommon)
        settingsCache = deepcopy(settings)
    elif 'settingsCache' not in globals() or settingsCache != settings:
